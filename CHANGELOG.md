...
- **A driving session now reads the handoff, not just its arrival (#99).** Four channels are forced out of every author. `remaining` and `testingNotes` reach an automated reader; `docsCandidates` and `decisions[].supersedes` reached nobody unless the maintainer acted by hand. Measured across twelve closed stories: **seventeen `docsCandidates` and fourteen `supersedes`, none actioned** — one file collected five candidates and had not been touched in sixteen days. ⚠️ **Sixteen of the seventeen named a `CLAUDE.md`**, which is the file class no role may write, so re-triggering the Writer on them was never going to work: a session is the only permitted reader. `take-on-story` now harvests each handoff at the wake that closes its task and discharges at the endgame — `docsCandidates` as a follow-up PR **after** the story's PR lands (never on the story branch, never mid-drive), `supersedes` reported rather than applied. `rules/claude-session.md` carries the matching bound at revision 15, and a test asserts every schema channel has a row in the driver's routing table.
